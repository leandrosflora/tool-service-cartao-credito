...
# Deliberately simpler than agent-runtime-renegotiation's tool-service: no journey_stage/
# journey_version/confirmation_message_id claims - those exist there to gate a stateful,
# multi-step journey with irreversible actions. Card limit/invoice lookups are stateless,
# read-only queries with nothing to sequence, so there's no journey to sign a stage into.
...
def _parse_execution_context(claims: dict[str, Any], tenant_id: str) -> ToolExecutionContext:
    ...
    if not all(isinstance(value, str) and value for value in (caller, conversation_id, message_id)):
        raise ValueError("Signed conversation, message, and caller claims are required.")
