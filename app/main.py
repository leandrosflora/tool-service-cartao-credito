...
def build_app():
    ...
    async def health_ready() -> JSONResponse:
        ...
        if settings.internal_auth_enabled:
            inbound_caller = "agent-runtime-fatura-cartao"
        ...
        try:
            async with httpx.AsyncClient(
                base_url=settings.core_bancario_base_url,
                timeout=2.0,
            ) as downstream:
                # core-bancario-mock has no dedicated /health endpoint - a 404 on an unreserved
                # path still proves the process is up and answering, which is all this check needs.
                response = await downstream.get("/clients/00000000000/card/limit")
                if response.status_code not in (200, 404):
                    failures.append("core_bancario_not_ready")
        except Exception:
            failures.append("core_bancario_unavailable")
...
if __name__ == "__main__":
    logger.info(
        "Starting tool-service-cartao-credito MCP server on %s:%s and REST server on %s:%s",
        settings.mcp_host,
        settings.mcp_port,
        settings.mcp_host,
        settings.docs_port,
