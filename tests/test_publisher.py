...
def test_publish_success_produces_keyed_message():
    ...
    publish_tool_executed_event(producer, make_settings(), "consultar_limite_cartao", "success", "corr-1")

    producer.produce.assert_called_once()
    args, kwargs = producer.produce.call_args
    assert args[0] == "tool.executed"
    assert kwargs["key"] == b"consultar_limite_cartao"
...
def test_publish_error_outcome():
    ...
    publish_tool_executed_event(producer, make_settings(), "consultar_fatura_cartao", "error", "corr-2")
...
def test_publish_never_includes_raw_arguments():
    ...
    publish_tool_executed_event(producer, make_settings(), "consultar_limite_cartao", "success", "corr-3")

    _, kwargs = producer.produce.call_args
    payload = json.loads(kwargs["value"])
    assert set(payload.keys()) == {"tenant_id", "tool_name", "outcome", "correlation_id"}
    assert "11111111111" not in kwargs["value"].decode("utf-8")
...
def test_publish_broker_unavailable_does_not_raise():
    ...
    publish_tool_executed_event(producer, make_settings(), "consultar_limite_cartao", "success", "corr-4")
